...
class Calendarro extends StatefulWidget {
  ...
  Calendarro({
    ...
    if (startDate == null) {
      this.startDate = DateUtils.getFirstDayOfCurrentMonth();
    } else {
      this.startDate = startDate;
    }
    this.startDate = DateUtils.toMidnight(this.startDate);

    if (endDate == null) {
      this.endDate = DateUtils.getLastDayOfCurrentMonth();
    } else {
      this.endDate = endDate;
    }
    this.endDate = DateUtils.toMidnight(this.endDate);

    if (this.startDate.isAfter(this.endDate)) {
      throw new ArgumentError("Calendarro: startDate is after the endDate");
    }
    startDayOffset = this.startDate.weekday - DateTime.sunday;
...
class CalendarroState extends State<Calendarro> {
  ...
  Widget build(BuildContext context) {
    if (widget.displayMode == DisplayMode.WEEKS) {
      ...
      pagesCount = DateUtils.calculateMonthsDifference(widget.startDate, widget.endDate) + 1;
    }

    pageView = PageView.builder(
      itemBuilder: (context, position) => _buildCalendarPage(position),
      itemCount: pagesCount,
      controller: PageController(initialPage: selectedSingleDate != null ? widget.getPageForDate(selectedSingleDate) : 0),
      onPageChanged: (page) {
    ...
    if (widget.displayMode == DisplayMode.WEEKS) {
      ...
      var maxWeeksNumber = DateUtils.calculateMaxWeeksNumberMonthly(widget.startDate, widget.endDate);
      widgetHeight = widget.dayLabelHeight + maxWeeksNumber * widget.dayTileHeight;
    }
  ...
  bool isDateSelected(DateTime? date) {
    switch (widget.selectionMode) {
      ...
      case SelectionMode.MULTI:
        final matchedSelectedDate =
            selectedDates!.firstWhere((currentDate) => DateUtils.isSameDay(currentDate!, date!), orElse: () => null);
      ...
      case SelectionMode.RANGE:
        switch (selectedDates!.length) {
          ...
          default:
            var dateBetweenDatesRange = (date!.isAfter(selectedDates![0]!) && date.isBefore(selectedDates![1]!));
            return DateUtils.isSameDay(date, selectedDates![0]!) || DateUtils.isSameDay(date, selectedDates![1]!) || dateBetweenDatesRange;
        }
    }
  }
  ...
  DateRange _calculatePageDateRangeInMonthsMode(int pagePosition) {
    ...
    if (pagePosition == 0) {
      ...
      DateTime firstDateOfCurrentMonth = DateUtils.addMonths(widget.startDate, pagePosition);
  ...
  DateRange _calculatePageDateRangeInWeeksMode(int pagePosition) {
    ...
    if (pagePosition == 0) {
      ...
      pageStartDate = DateUtils.addDaysToDate(widget.startDate, 7 * pagePosition - widget.startDayOffset);
      pageEndDate = widget.endDate;
    } else {
      pageStartDate = DateUtils.addDaysToDate(widget.startDate, 7 * pagePosition - widget.startDayOffset);
      pageEndDate = DateUtils.addDaysToDate(widget.startDate, 7 * pagePosition + 6 - widget.startDayOffset);
    }
  ...
  void _setMultiSelectedDate(DateTime? date) {
    final alreadyExistingDate = selectedDates!.firstWhere((currentDate) => DateUtils.isSameDay(currentDate!, date!), orElse: () => null);
